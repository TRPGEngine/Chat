...
function initFunction() {
  ...
  app.chat = {
    ...
    findMsgAsync: async function(msg_uuid) {
      ...
      let res = null;
      try {
        res = await db.models.chat_log.oneAsync({uuid: msg_uuid});
      } finally {
        db.close();
      }
      return res;
    ...
    saveChatLogAsync: async function() {
      ...
      let res = null;
      try {
        res = await db.models.chat_log.createAsync(cacheList);
    ...
    getChatLogSumAsync: async function() {
      ...
      let res = null;
      try {
        res = await db.models.chat_log.countAsync();
      }finally {
        db.close();
      }
      return res;
    ...
    getChatLogAsync: async function(page=1, limit=10) {
      ...
      let res = null;
      try {
        res = await db.models.chat_log.find().offset((page-1)*limit).limit(limit).findAsync();
      }finally {
        db.close();
      }
      return res;
    ...
    notifyUpdateMsg: function(converseUUID, isGroup, payload) {
      ...
      if(isGroup) {
        ...
        if(player1) {
          console.log('socket', player1.socket);
...
function initData() {
  ...
  app.storage.connect(function(db) {
    db.models.chat_converse.find({}, function(err, converses) {
      if(!!err) {
        console.error('init converse error');
      }else {
