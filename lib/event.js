...
let message = function message(data, cb) {
  ...
  if(!!app.player) {
    ...
    let _pkg = {message, room, sender_uuid, to_uuid, type, is_public, date, uuid: _uuid, data: _data};
...
let getConverses = async function getConverses(data, cb) {
  ...
  try {
    ...
    let user = player.user;
    let db = await app.storage.connectAsync();
    let converses = await db.models.chat_converse.findAsync({'owner_id': user.id});
    converses.push({
      uuid: 'trpgsystem',
      type: 'system',
      name: '系统消息',
...
let updateSystemCardChatData = async function updateSystemCardChatData(data, cb) {
  let app = this.app;
  let socket = this.socket;
  ...
  try {
    ...
    for (let l of logs) {
      if(l.uuid === chatUUID && l.type === 'card' && l.sender_uuid === 'trpgsystem' && l.to_uuid===player.uuid) {
        log = l;
      }
    }
