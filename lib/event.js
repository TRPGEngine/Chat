...
let getUserChatLog = async function getUserChatLog(data, cb, db) {
  ...
  // IDEA: 定义: 用户间会话无视掉自身发送的tip类型信息
  let conditions = {
    converse_uuid: null,
    or: [
      {sender_uuid: userUUID, to_uuid: selfUUID},
      {sender_uuid: selfUUID, to_uuid: userUUID, type: app.storage._orm.ne('tip')},
    ],
  }
...
let getAllUserConverse = async function getAllUserConverse(data, cb, db) {
  ...
  let ret1 = await db.models.chat_log.aggregate({
    sender_uuid: app.storage._orm.not_like('trpg%'),
    to_uuid: player.uuid,
    converse_uuid: null,
    is_group: false,
  }).distinct('sender_uuid').getAsync();
  let ret2 = await db.models.chat_log.aggregate({
    sender_uuid: player.uuid,
    to_uuid: app.storage._orm.not_like('trpg%'),
    converse_uuid: null,
    is_group: false,
  }).distinct('to_uuid').getAsync();
  let senders = [...ret1[0], ...ret2[0]];
  // 数组去重
  senders = Array.from(new Set(senders));
  cb({result: true, senders});
}
...
let getOfflineUserConverse = async function getOfflineUserConverse(data, cb, db) {
  ...
  senders = [...senders, ...ret[0]];
  senders = Array.from(new Set(senders));// 数组去重
  cb({ result: true, senders });
}
